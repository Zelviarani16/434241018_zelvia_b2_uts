  ...
  Future<void> _saveChanges() async {
    ...
    try {
      ...
      if (mounted) {
        ...
        // Jangan reset field - biarkan dropdown menunjukkan nilai terakhir yang dipilih
        // Field akan di-update saat data ticket di-refresh dari API
      }
    } finally {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: ticketAsync.when(
        ...
        data: (ticket) {
          // Valid statuses untuk helpdesk
          const _helpdeskStatuses = ['in_progress', 'resolved', 'closed'];

          // Inisialisasi atau clamp _selectedStatus ke nilai yang valid untuk role ini
          if (role == 'helpdesk') {
            if (_selectedStatus == null ||
                !_helpdeskStatuses.contains(_selectedStatus)) {
              // Jika status tiket valid untuk helpdesk, pakai itu; kalau tidak (misal 'open'), pakai 'in_progress'
              _selectedStatus = _helpdeskStatuses.contains(ticket.status)
                  ? ticket.status
                  : 'in_progress';
            }
          } else {
          ...
          // Nilai aman untuk dropdown helpdesk (selalu ada di list)
          final helpdeskDropdownValue =
              _helpdeskStatuses.contains(_selectedStatus)
              ? _selectedStatus
              : 'in_progress';
          return SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                ...
                // ── Panel Admin (kelola tiket + assign helpdesk) ──
                if (role == 'admin') ...[
                  ...
                  _Card(
                    ...
                    child: Column(
                      children: [
                        ...
                        // Dropdown assign ke helpdesk
                        Consumer(
                          builder: (context, ref, _) {
                            final helpdeskAsync = ref.watch(
                              helpdeskUsersProvider,
                            );
                            return helpdeskAsync.when(
                              loading: () => const LinearProgressIndicator(),
                              error: (e, _) => Text(
                                'Gagal memuat helpdesk: $e',
                                style: TextStyle(
                                  color: textSecondary,
                                  fontSize: 12,
                                ),
                ...
                // ── Panel Helpdesk (FR-006 pt 4: update status, pt 6: tutup tiket) ──
                if (role == 'helpdesk') ...[
                  _SectionLabel(
                    label: 'Kelola Tiket (Ditugaskan)',
                    textColor: textPrimary,
                  ),
                  const SizedBox(height: 10),
                  _Card(
                    isDark: isDark,
                    borderColor: const Color(0xFF4F7EFF).withOpacity(0.3),
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        // Info: helpdesk hanya bisa update status tiket yg ditugaskan
                        Container(
                          ...
                          child: Row(
                            children: [
                              ...
                              Expanded(
                                child: Text(
                                  'Anda dapat mengubah status dan menutup tiket ini.',
                                  style: TextStyle(
                                    fontSize: 11,
                                    color: isDark
                        ...
                        // Dropdown update status — FR-006 point 4
                        // Gunakan helpdeskDropdownValue (sudah divalidasi) bukan _selectedStatus langsung
                        DropdownButtonFormField<String>(
                          key: const ValueKey('dropdown_helpdesk_status_v2'),
                          value: helpdeskDropdownValue,
                          dropdownColor: isDark
                              ? const Color(0xFF252839)
                              : Colors.white,
                          style: TextStyle(color: textPrimary, fontSize: 14),
                          decoration: InputDecoration(
                            labelText: 'Update Status Pengerjaan',
                            labelStyle: TextStyle(
                              color: textSecondary,
                              fontSize: 13,
                            ),
                          ...
                          // Helpdesk dapat set: in_progress, resolved, closed
                          items: _helpdeskStatuses
                              .map(
                                (s) => DropdownMenuItem(
                                  value: s,
                                  child: Text(
                        ...
                        // Tombol Simpan — FR-006 point 4 & 6
                        SizedBox(
                          width: double.infinity,
                          height: 46,
                          child: ElevatedButton.icon(
                            ...
                            children: [
                              CircleAvatar(
                                ...
                                backgroundColor: isUser
                                    ...
                                    : const Color(0xFF4F7EFF).withOpacity(0.15),
                                child: Text(
                ...
                Container(
                  ...
                  child: Row(
                    ...
                    children: [
                      ...
                      GestureDetector(
                        onTap: () =>
                            _sendComment(role, authState.user?.name ?? 'User'),
                        child: Container(
                          width: 38,
                          height: 38,
                          decoration: BoxDecoration(
